...
$query = 'insert into  daftarvariables (derj, derjNow, dateOfDerj) values("'.$derjValue.'", "'.$_POST['derjVa'].'", "'.$currentDate.'")';
...
while ($row = mysqli_fetch_array($result2)) {
	...
	$derjValueNow=$row['derjNow'];
} 



$sql3 = "SELECT * FROM daftarvariables WHERE DATE(dateOfDerj) = DATE(NOW() - INTERVAL 1 DAY) ORDER BY daftarVar_id DESC LIMIT 1";
...
$sql3 = "SELECT * FROM daftarvariables ORDER BY daftarVar_id DESC LIMIT 1";
$result3 = mysqli_query($connection, $sql3);
while ($row = mysqli_fetch_array($result3)) {
$derjValueYesterday=$row['derj'];
...
function getPrevVal()
{
	global $connection, $PrevVal;
    static $getPrevVal_called = false;
    if (!$getPrevVal_called) {
		$getPrevVal_called = true;
		$sql4 = "SELECT * FROM daftarM WHERE DATE(dateOfToday) = DATE(NOW() - INTERVAL 1 DAY) ORDER BY daftar_id DESC LIMIT 1";
...
if(isset($rasseedPrevValue)&& $rasseedPrevValue!=0)
{
	...
	$sql4 = "SELECT * FROM daftarM ORDER BY daftar_id DESC LIMIT 1";
$result4 = mysqli_query($connection, $sql4);
while ($row = mysqli_fetch_array($result4)) {
	$PrevVal=$row['baki'];
echo $PrevVal;
...
echo "prevVal: ".getPrevVal();
	...
	echo '<input type="text" name="derjVa" id="derjVa" style="width:150px;border-color:#0000CD;" value="'.$derjValueNow.'"></input>';
...
echo "</td>";
	...
	echo '<input type="text" name="raseedPrev" id="raseedPrev" style="width:150px;border-color:#0000CD;" value="'. $PrevVal .'"></input>';
...
<form id="mainForm" name="mainForm" method="post" action="daftar.php">
<table id="header">
...
<input type="hidden" id="postedPrev" name="postedPrev" value="<?php
...
echo "</td>";
 ...
 //
echo isset($_POST['postedPrev'])? $POST['postedPrev']:0;
 echo "<br><br>";
  echo "mabe3Today".$mabee3Today;
echo "PrevValCur".(isset($PrevVal)? $PrevVal:0);
 echo "sumOfMasrofat".$sumOfMasrofat;
 
 //
 
 $baki=$mabee3Today+$PrevVal-$sumOfMasrofat;
